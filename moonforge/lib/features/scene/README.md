...
The Scene feature in Moonforge provides comprehensive tools for managing scenes within adventures. Scenes are the smallest narrative units, representing specific locations, events, or encounters.
...
#### SceneScreen
- **Location**: `lib/features/scene/views/scene_screen.dart`
...
#### SceneEditScreen
- **Location**: `lib/features/scene/views/scene_edit_screen.dart`
...
final scenes = await sceneService.getScenesByAdventure(adventureId);
...
final errors = SceneValidators.validateScene(scene);
if (errors.isEmpty) {
  // Scene is valid
}
