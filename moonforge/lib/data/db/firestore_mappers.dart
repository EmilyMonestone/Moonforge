...
CampaignsCompanion campaignFromFirestore(String id, Map<String, dynamic> d) =>
  CampaignsCompanion(
    ...
    memberUids: drift.Value(((d['memberUids'] ?? <String>[]) as List).cast<String>()),
    entityIds: drift.Value(((d['entityIds'] ?? <String>[]) as List).cast<String>()),
...
PartiesCompanion partyFromFirestore(String id, Map<String, dynamic> d) =>
  PartiesCompanion(
    ...
    name: drift.Value(d['name'] as String),
    summary: drift.Value(d['summary'] as String?),
    memberEntityIds: drift.Value(((d['memberEntityIds'] ?? <String>[]) as List?)?.cast<String>()),
    createdAt: drift.Value((d['createdAt'] as Timestamp?)?.toDate()),
    updatedAt: drift.Value((d['updatedAt'] as Timestamp?)?.toDate()),
    rev: drift.Value((d['rev'] as int?) ?? 0),
  );
...
EncountersCompanion encounterFromFirestore(String id, Map<String, dynamic> d) =>
  EncountersCompanion(
    ...
    combatants: drift.Value(
      ((d['combatants'] ?? <Map>[]) as List).map((e) => Map<String, dynamic>.from(e as Map)).toList()
    ),
    entityIds: drift.Value(((d['entityIds'] ?? <String>[]) as List).cast<String>()),
    createdAt: drift.Value((d['createdAt'] as Timestamp?)?.toDate()),
    updatedAt: drift.Value((d['updatedAt'] as Timestamp?)?.toDate()),
...
EntitiesCompanion entityFromFirestore(String id, Map<String, dynamic> d) =>
  EntitiesCompanion(
    ...
    statblock: drift.Value(Map<String, dynamic>.from((d['statblock'] ?? {}) as Map)),
    ...
    content: drift.Value(d['content'] as Map<String, dynamic>?),
    images: drift.Value(
      ((d['images'] ?? <Map>[]) as List?)?.map((e) => Map<String, dynamic>.from(e as Map)).toList()
    ),
    createdAt: drift.Value((d['createdAt'] as Timestamp?)?.toDate()),
    updatedAt: drift.Value((d['updatedAt'] as Timestamp?)?.toDate()),
    rev: drift.Value((d['rev'] as int?) ?? 0),
...
MediaAssetsCompanion mediaAssetFromFirestore(String id, Map<String, dynamic> d) =>
  MediaAssetsCompanion(
    id: drift.Value(id),
    filename: drift.Value(d['filename'] as String),
    size: drift.Value(d['size'] as int),
    mime: drift.Value(d['mime'] as String),
    captions: drift.Value(((d['captions'] ?? <String>[]) as List?)?.cast<String>()),
    alt: drift.Value(d['alt'] as String?),
    variants: drift.Value(
      ((d['variants'] ?? <Map>[]) as List?)?.map((e) => Map<String, dynamic>.from(e as Map)).toList()
    ),
    createdAt: drift.Value((d['createdAt'] as Timestamp?)?.toDate()),
    updatedAt: drift.Value((d['updatedAt'] as Timestamp?)?.toDate()),
    rev: drift.Value((d['rev'] as int?) ?? 0),
