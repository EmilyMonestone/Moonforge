...
class _AdaptiveBreadcrumbState extends State<AdaptiveBreadcrumb> {
  ...
  List<Widget> _buildBreadcrumbItems() {
    ...
    if (totalItems <= 2 || !_hasOverflow) {
      ...
      // Show last item
      items.add(_buildClickableItem(widget.items[totalItems - 1], totalItems - 1));
    }
  ...
  void _checkOverflow(double maxWidth) {
    ...
    double requiredWidth = itemWidths.first + itemWidths.last + dividerWidth * 2 + ellipsisWidth;
